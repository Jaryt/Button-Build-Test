...
class FormData {
  ...
  _isBoundary(line) {
    return this.boundary.equals(line); // eslint-disable-line no-extra-parens
  }

  _isBoundaryEnd(line) {
    return this._boundaryEnd.equals(line); // eslint-disable-line no-extra-parens
  }
  ...
  parseBody(body) {
    ...
    for (const line of lines) {
      if (this._isBoundary(line)) {
        ...
        if (line.length) currentEntry.setHeader(line.toString()); // eslint-disable-line no-extra-parens
        else state = ParserState.inBody;
      } else if (state === ParserState.inEpilogue) bufferUtils.appendLine(this._epilogue, line);else if (state === ParserState.inBody) bufferUtils.appendLine(currentEntry.body, line); // eslint-disable-line no-extra-parens
